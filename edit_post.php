    ...
    if(mysqli_num_rows($res) > 0){
        while ($row = mysqli_fetch_assoc($res)){
            ...
            echo "<div class='w3-section'><label>Title</label>
                  <input class='w3-input w3-border' type='text' name='title' placeholder='post title here' value='$title' required></div>";
            echo "<div class='w3-section'><label>Content</label>
                 <textarea class='w3-input w3-border' type='textarea' name='content' placeholder='content' rows='10' cols='30'required>$content</textarea></div>";
        }
    }
?>     
